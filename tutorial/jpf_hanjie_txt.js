...
//Run the init() function, insert the following statementto run the fucntion when the window is loaded
window.onload = init;
...
function init() {
    ...
    document.getElementById("puzzleTitle").interHTML = "Puzzle 1";

    //Insert the HTML code for thew first puzzle
...
function swapPuzzle(e) {
    ...
    //Switch statement that calls drawPuzzle() for each three possible puzzle ID value
    switch (puzzleID) {
        case "puzzle1":
            document.getElementById("puzzle").innerHTML =
                drawPuzzle(puzzle1Hint, puzzle1Rating, puzzle1);
        ...
        function() {
            ...
            setTimeout(
                function() {
                    ...
                    for (var i = 0; i < puzzleCells.length; i++) {
                        if (puzzleCells[i].style.background === "rgb(255, 211, 211)") {
                            puzzleCells[i].style.backgroundColor = "rgb(255, 255, 255)";
                        }
                        if (puzzleCells[i].style.background === "rgb(255, 101, 101)") {
                            puzzleCells[i].style.background = "rgb(101, 101, 101)";
                        };
                    }
                }, 500
            );
...
function endBackground() {
    //Remove the event listener for every puzzle
    for (var i = 0; i < puzzleCells.length; i++) {
        puzzleCells[i].removeEventListener("mouseenter", extendBackground);

        //Cursor style change when mouse is released
        puzzleCells[i].style.cursor = "url(jpf_pencil.png), pointer";
    }
}
